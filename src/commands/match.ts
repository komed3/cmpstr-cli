...
export async function match (
  ...
  const {
    async = false, verbose = false, list = false, delimiter = ',', metric = 'levenshtein',
    flags = '', threshold = 0, sort = 'desc', n = Infinity
  } = config;
  ...
  let res = async ? await cmp.matchAsync( A, B, threshold ) : cmp.match( A, B, threshold );
  res = res.sort( ( a: any, b: any ) => verbose
    ? sort === 'asc' ? a.res - b.res : b.res - a.res
    : sort === 'asc' ? a.match - b.match : b.match - a.match
  ).slice( 0, n );

  await output( config, cmd, verbose ? JSON.stringify( res ) : res );
}
